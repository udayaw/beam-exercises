...
public class MyPipeline {
    ...
    /**
     * The {@link Options} class provides the do_fn execution options passed by the
    ...
    static class  JsonToCommonLog extends DoFn<String, CommonLog> {
        ...
        public void processElement(@Element String json, OutputReceiver<CommonLog> r) throws Exception {
            Gson gson = new Gson();
            CommonLog log = gson.fromJson(json, CommonLog.class);
            r.output(log);
        }
    }
    ...
    public static PipelineResult run(Options options) {
        ...
        pipeline.apply("ReadLines", TextIO.read().from("gs://qwiklabs-gcp-04-e8dd0c31bf84/events.json"))
                .apply(ParDo.of(new JsonToCommonLog()))
                .apply(BigQueryIO.<CommonLog>write()
                        .to("qwiklabs-gcp-04-e8dd0c31bf84:logs.logs")
                        .useBeamSchema()
